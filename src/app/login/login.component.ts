...
export class LoginComponent implements OnInit {
  ...
  private _animate(): void {
    ...
    function draw() {
      ...
      fxH += STEP;
      drawLayer("#8ab9ff", (path: Path2D): void => {
        path.lineTo(sineEaseInOut(innerH + fxH + 100, 0, 600, 1000), innerH);
        path.bezierCurveTo(
          sineEaseInOut(fxH, 0, 300, 800),
          200,
          sineEaseInOut(fxH, 0, 500, 1000),
          60,
          50,
          0,
        );
      });
      drawLayer("#356da9", (path: Path2D): void => {
        path.lineTo(sineEaseInOut(innerH + fxH + 200, 0, 600, 1000), innerH);
        path.bezierCurveTo(
          sineEaseInOut(fxH, 0, 200, 900),
          200,
          sineEaseInOut(fxH, 0, 400, 1000),
          60,
          50,
          0,
        );
      });
      drawLayer("#30333e", (path: Path2D): void => {
        path.lineTo(sineEaseInOut(innerH + fxH, 0, 500, 800), innerH);
        path.bezierCurveTo(
          sineEaseInOut(fxH, 0, 100, 1000),
          200,
          sineEaseInOut(fxH, 0, 300, 1000),
          60,
          50,
          0,
        );
