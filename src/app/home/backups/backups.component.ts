...
export class BackupsComponent implements OnInit, OnDestroy {
  ...
  public restoreBackup(hash: string): void {
    ...
    from(this._electron.ipcRenderer.invoke('message-box', dialogOpts))
      .pipe(
        switchMap((val) => iif(() => val.response == 0, this._api.restoreBackup(hash), throwError(() => new Error('Cancelled by user')))),
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error(err);
          this._toast.show('danger', `Бэкап файла ${this.$current.value.file.name} не был установлен по причине:`, err.message, faClipboard);
        }
      });
  }
