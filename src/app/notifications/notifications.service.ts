...
export class NotificationsService {
  ...
  subToNotifications() {
    const subscriptions: TeardownLogic[] = [
      this._ws
        .getAlertGuardBlockOn()
        .pipe(
          filter(
            () => !!JSON.parse(window.localStorage.getItem("alerts")).autoBlock,
          ),
        )
        .subscribe((line) => {
          this.spawnNotification(
            `Кикбан ${line.nickname}`,
            `${line.nickname} заблокирован системой по причине ${line.content.message}`,
          );
        }),
      this._ws
        .getAlertKickban()
        .pipe(
          filter(
            () => !!JSON.parse(window.localStorage.getItem("alerts")).autoBan,
          ),
        )
        .subscribe((line) => {
          this.spawnNotification(
            ...
            `${
              line.nickname
            } кикнут системой по причине: ${getProcessTranslation(
              line.process,
            )}`,
          );
        }),
      this._ws
        .getServerStopNotification()
        .pipe(
          filter(
            () =>
              !!JSON.parse(window.localStorage.getItem("alerts"))
                .serverShutdown,
          ),
        )
        .subscribe((user: any) => {
          this.spawnNotification(
            "Сервер остановлен",
            `${user.username} остановил работу сервера`,
            user.user_avatar,
          );
        }),
      this._ws
        .getServerRebootNotification()
        .pipe(
          filter(
            () =>
              !!JSON.parse(window.localStorage.getItem("alerts"))
        ...
        .subscribe((user: any) => {
          this.spawnNotification(
            ...
            `${user.username} запустил перезагрузку сервера`,
            user.user_avatar,
          );
        }),
    ];
