...
export class MapViewerEffects implements OnInitEffects {
  fetchMapsDirectory$ = createEffect(() =>
    this.actions$.pipe(
      ofType(mapViewerActions.fetchFileTree),
      switchMap(() =>
        this.api.getMapsDir().pipe(
  ...
  saveMapXMLDocumentLocally$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([, objects, name]) => [this.mapViewerService.mapObjectsToXML(objects), name]),
      switchMap(([xmlDocument, name]: [XMLDocument, string]) =>
        from(
  ...
  saveMapXMLDocumentLocallyError$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        tap(([err, name]) =>
          this.toast.show('danger', `Файл ${name} не был сохранен сохранен`, err)
        )
      ),
    { dispatch: false }
  );
  ...
  saveXMLDocumentOnServer$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([, objects, path]) => [this.mapViewerService.mapObjectsToXML(objects), path]),
      switchMap(([xmlDocument, path]: [XMLDocument, string]) =>
        from(
          this.electron.ipcRenderer.invoke('message-box', {
            type: 'warning',
