...
export class WebSocketService {
  ...
  protected activies: Observable<any> = this.getUserActitvity().pipe(
  ...
  constructor(
    ...
    this._router.events
      .pipe(filter((val: any) => val instanceof NavigationEnd))
      .subscribe((val: NavigationEnd) => {
        if (val.url) {
          switch (val.url) {
            case "/home/dash": {
              _socket.emit("user-action", UserActivity.IDLE);
              break;
            }
            case "/home/search": {
              _socket.emit("user-action", UserActivity.IN_LOGS);
              break;
            }
            case "/home/configs": {
              _socket.emit("user-action", UserActivity.REDACT);
              break;
            }
            case "/home/maps": {
              _socket.emit("user-action", UserActivity.IN_MAPS);
              break;
            }
            case "/home/banhammer": {
              _socket.emit("user-action", UserActivity.IN_BANS);
              break;
            }
            case "/home/admins": {
              _socket.emit("user-action", UserActivity.IN_ADM);
              break;
            }
            case "/home/backups": {
              _socket.emit("user-action", UserActivity.IN_BACKS);
              break;
            }
            case "/home/stats": {
              _socket.emit("user-action", UserActivity.IN_STATS);
              break;
            }
            case "/login": {
              this.disconnect();
    ...
    _socket.on("connect", () => {
      console.log(
        "%c[socket-service]",
        "color: tomato",
        `Connected to Liberty-Admin-Node`,
      );
    });
    ...
    this._injector
      .get(UserService)
      .loggedInUser$.pipe(filter((user) => !!user))
      .subscribe((user) => {
        this.connect();
      });
  }
  ...
  getUserActitvity(): Observable<any> {
    return this._socket.fromEvent("user-activity").pipe(map((data) => data));
  }
