...
export class ConfigsService {
  ...
  saveFileAsBlob(path: string, blob: Blob): Observable<any> {
    ...
    return this._api.saveFile(form).pipe(
      tap((event) => {
        if (event.type === HttpEventType.UploadProgress)
          this.dprogress$.next(Math.round((100 * event.loaded) / event.total));
      }),
  ...
  public downloadFile(path: string): void {
    ...
    this._electron.ipcRenderer
      ...
      .catch((res) => {
        this._toast.show(
          "warning",
          `Файл <b>${res.filePath}</b> не был загружен`,
          res.message,
          faInfo,
        );
        console.error(res);
    ...
    from(this._electron.ipcRenderer.invoke("message-box", dialogOpts))
      ...
      .subscribe({
        next: () => {
          this._toast.show(
            "success",
            `Файл <b>${path}</b> удален с сервера`,
            null,
            faTrash,
          );
        },
        error: (err) => {
          this._toast.show(
            "danger",
            `Файл <b>${path}</b> не был удален с сервера`,
            err,
            faTrash,
          );
        },
  ...
  pathToClipboard(path: string): void {
    this._electron.ipcRenderer.invoke("clipboard", path).then(() => {
      this._toast.show(
        "success",
        "Путь скопирован в буффер обмена",
        null,
        faCopy,
      );
    });
