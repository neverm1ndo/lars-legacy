...
export class TextEditorComponent implements OnInit, AfterViewInit, OnDestroy {
      ...
      switch (event.code) {
        ...
        case "Space": {
          this._zone.runOutsideAngular(() => {
            this.editor.codeMirrorGlobal.commands.autocomplete(
  ...
  @HostListener("mousewheel", ["$event"]) wheelEvent(event: WheelEvent) {
    if (event.ctrlKey) {
      ...
      this.editorStyle.nativeElement.style.fontSize =
        String(size + event.deltaY / 100) + "px";
  ...
  searchIn(): void {
    this._zone.runOutsideAngular(() => {
      this.editor.codeMirrorGlobal.commands.find(
        this.editor.codeMirror,
        this.query.find,
      );
  ...
  saveFile(): void {
    const blob = new Blob([this.textplain], {
      type: this.configs.stats$.getValue().mime,
    });
    this.configs
  ...
  ngAfterViewInit(): void {
    ...
    this._zone.runOutsideAngular(() => {
      this.editor.codeMirrorGlobal.autocomplete = (_cm: any) => {
        this.editor.codeMirror.showHint({
          hint: this.editor.codeMirrorGlobal.hint.anyword,
        });
      };
    });
  }
