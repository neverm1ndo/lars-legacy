...
export class SearchComponent implements OnInit, OnDestroy {
  ...
  public $newLines: Observable<number> = merge(
    this._ws.getNewLogLines().pipe(map(() => 1)),
    this.syncronize.asObservable().pipe(map(() => 0)),
  ).pipe(scan((acc, val) => (val === 0 ? 0 : acc + val), 0));
  ...
  sendQuery(): void {
    if (this.searchForm.valid) {
      ...
      this._api.addToRecent(HistoryListEnum.SEARCH, {
        q: this.query.query,
        date: Date.now(),
      });
    } else {
