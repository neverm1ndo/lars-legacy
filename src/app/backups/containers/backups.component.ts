import { Component, OnInit, OnDestroy, ChangeDetectionStrategy, ViewChild, AfterViewInit, Optional } from '@angular/core';
...
export class BackupsComponent implements OnInit, OnDestroy, AfterViewInit {
	...
	public removeBackup(): void {
		...
		this.translateService.get([
		...
		], { filename: current.file.name, path: current.file.path }).pipe(
			...
			switchMap(
				(val) => iif(
					...
					this.translateService.get('Backups.Dialog.ClientResponseRejectError').pipe(
		...
		).subscribe(
			{
				next: (backups: Backup[]) => {
					...
					this._toast.show('success',
						this.translateService.instant('Backups.Toast.DeleteSuccess', { filename: current.file.name }),
						current.file.path, 
						faClipboardCheck
					);
					this.closeView();
				...
				error: (err) => {
					console.error(err);
						this._toast.show('danger',
						this.translateService.instant('Backups.Toast.DeleteError', { filename: current.file.name }),
						err.message,
						faClipboard
					);
				}
	...
	this.$currentSubject.pipe(
		map(({ file }) => file),
		switchMap((file) => this.translateService.get([
		...
		switchMap((options: Electron.MessageBoxOptions) => from(this._electron.ipcRenderer.invoke('message-box', options))),
		switchMap(
			(val: Electron.MessageBoxReturnValue) => iif(
				() => val.response === 1,
	...
	).subscribe(
		{
			next: () => {
				this._toast.show(
					'success',
					this.translateService.instant('Backups.Toast.InstallSuccess', { filename: current.file.name }),
					current.file.path,
					faClipboardCheck
				);
			},
			error: (err) => {
				...
				this._toast.show(
					'danger',
					this.translateService.instant('Backups.Toast.InstallError', { filename: current.file.name }),
					err.message,
					faClipboard
				);
			}
