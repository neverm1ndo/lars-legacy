...
export class MapsComponent implements OnDestroy {
  ...
  private setPanesState(): number[] {
    try {
      const states = JSON.parse(
        window.localStorage.getItem("lars/ui/panes/maps"),
      );
  ...
  public addNewMap(event: any): void {
    ...
    this._api.uploadFileMap(formData).subscribe({
      next: (event) => {
        if (event.type === HttpEventType.UploadProgress) {
          ...
          this._toast.show(
            "success",
            `Карта <b>${files[0].name}</b> успешно добавлена`,
            null,
            faSave,
          );
      ...
      error: (err) => {
        this._toast.show(
          "warning",
          `Карта <b>${files[0].name}</b> не была добавлена, или она добавилась, но сервер вернул ошибку`,
          err.message,
          faInfo,
        );
      },
