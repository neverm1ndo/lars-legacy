...
export class MapsService {
  ...
  public mapGroupToXML(mapGroup: Group): XMLDocument {
    ...
    for (let object of mapGroup.children) {
      const objectElement: Element = resultXML.createElementNS(
        xmlNamespaceURI,
      ...
      [rotX, rotY, rotZ] = [rotX, rotY, rotZ].map((rad: number) =>
        MathUtils.radToDeg(rad),
      );
    ...
    const postComment: Comment = resultXML.createComment(
      `\n\tLARS gta-liberty.ru\n\tLast edited by ${
        this._user.loggedInUser$.value.username
  ...
  public mapToObject(xml: string): MapObject[] {
    ...
    try {
      const map = parser.parseFromString(
        xml.replace(xmlfyRegex, ""),
        "application/xml",
      );

      const parseerror: Element = map
      ...
      if (!mapElement)
        throw new Error(
          "MAP_TAG_IS_MISSING: map tag is not exists in this document",
        );
  ...
  public saveMapCloud(path: string, group: Group): Observable<unknown> {
    ...
    return from(
      ...
      map(() => {
        ...
        const xmlDocumentAsString: string =
          this.mapGroupToXML(group).toString();

        const blob: Blob = new Blob([xmlDocumentAsString], {
  ...
  public saveMapLocal(name: string, group: Group): Observable<unknown> {
    ...
    return from(
      this._electron.ipcRenderer.invoke("save-dialog", saveDialogOpts),
    ).pipe(
      filter((res: SaveDialogReturnValue) => res.filePath && !res.canceled),
      switchMap((res) =>
        from(
          this._electron.fs.promises.writeFile(
            res.filePath,
            this.mapGroupToXML(group).toString(),
          ),
        ),
  ...
  public getMap(path: { path: string; name?: string }): Observable<any> {
    return this._api.getMap(path.path).pipe(
  ...
  public mapNotLoaded(err: Error) {
    this._toast.show(
      "danger",
      `Карта не был загружена по причине:`,
      err.message,
      faExclamationTriangle,
    );
  }
