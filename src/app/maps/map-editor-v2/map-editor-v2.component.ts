...
export class MapEditorV2Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  @Input("map") private set _mapObjects(objects: MapObject[]) {
    const clean = objects.filter(
      (object) => object.name == "object" && object.model,
    );
  ...
  private _resizeObserver: ResizeObserver = new ResizeObserver(
    (_entries: ResizeObserverEntry[]) => {
      this.canvas.style.width =
        this.canvas.parentElement.parentElement.clientWidth + "px";
      this.canvas.style.height =
        this.canvas.parentElement.parentElement.clientHeight + "px";
  ...
  private readonly _resourcesPackSettings: ResourcesPackSettings =
    this._loadResouresPackSettings();
  ...
  private _showDebugGUI(): void {
    ...
    createFolder("Chunks", [
      ...
      [
        this.loadedMapGroup.children,
        "length",
        0,
        this._mapChunksNames.length + 2,
      ],
    ]);
    createFolder(
      "Camera",
    ...
    createFolder(
      "Memory",
      ["geometries", "textures"].map((type: string) => [
        this._renderer.info.memory,
        type,
      ]),
      renderer,
    );
  ...
  private _handleRaycasterIntersectMapObjects(): void {
    ...
    const intersects = this._cursorRayCaster.intersectObjects(
      this.loadedMapGroup.children,
    );
  ...
  private _focusObject(object: THREE.Object3D) {
    ...
    const intersectedObjectDiv = this._createTextLabel(
      `${object.parent.userData.id}\n(${object.parent.userData.model})`,
    );

    const intersectedObjectLabel: CSS2DObject = new CSS2DObject(
      intersectedObjectDiv,
    );
    intersectedObjectLabel.position.set(
      this.selectedMapObject.position.x,
      this.selectedMapObject.position.y,
      this.selectedMapObject.position.z,
    );
  ...
  private _loadModelFromFileSystem(name: string): Observable<THREE.Group> {
    ...
    const resourcesPackURI: string = encodeURI(
      `${protocol}://${host}${textures}`,
    );
    return from<Promise<string>>(
      this._electron.ipcRenderer.invoke(
        "model",
        path.join(host, `${name}.mtl`),
      ),
    ).pipe(
      map((buffer: string) =>
        this._mtlLoader.setResourcePath(resourcesPackURI).parse(buffer, ""),
      ),
      ...
      switchMap((materials: MTLLoader.MaterialCreator) =>
        from(
          this._electron.ipcRenderer.invoke(
            "model",
            path.join(host, `${name}.obj`),
          ),
        ).pipe(
          map((buffer: Buffer) =>
            this._objectLoader.setMaterials(materials).parse(buffer.toString()),
          ),
        ),
      ),
  ...
  private _loadMapChunk(name: string): Observable<[THREE.Group, string]> {
    return this._loadModelFromFileSystem(name).pipe(
  ...
  private _unloadMapObjects(): void {
    this._scene.remove(
      ...this._mapObjects.map((object: MapObject) =>
        this._scene.getObjectByName(object.name),
      ),
    );
    this._mapObjects = [];
  }
  ...
  private _loadMapObjects(): Observable<[THREE.Group, string]> {
    return concat(
      ...this._mapObjects.map((object: MapObject) =>
        this._loadMapChunk(this._getMapObjectNameById(object.id)),
      ),
  ...
  private _createScene(): void {
    ...
    this._scene.fog = new THREE.Fog(
      COLOR.SKY,
      this._farClippingPlane - 150,
      this._farClippingPlane,
    );

    const _loadMapChunks: Observable<[THREE.Group, string]> =
      this.__loadMapChunks().pipe(
        tap(([chunk, name]) => {
          chunk.name = name;
    ...
    _loadMapChunks.pipe(mergeMap(() => this.__loadMapObjects())).subscribe({
      next: (group) => {
        this.loadedMapGroup.add(group);

        if (
          this.loadedMapGroup.children.length !== this.$mapObjects.value.length
        )
          return;
  ...
  private __loadMapChunks() {
    const chunks = this._mapChunksNames.map((name: string) =>
      this._loadMapChunk(name),
    );
  ...
  private _chunkBoundingContainsCamera(): void {
    ...
    for (let box of boxes) {
      ...
      if (bounding.containsPoint(this._camera.position))
        this._scene.add(this._loadedTerrainChunks.get(name));
  ...
  private _renderingLoop(): void {
    ...
    const gammaCorrectionPass: ShaderPass = new ShaderPass(
  ...
  private _update(): void {
    ...
    const render = (): void => {
      ...
      if (DELTA > INTERVAL) {
        ...
        if (this._terrainBoundingBoxes.size == this._mapChunksNames.length)
          this._chunkBoundingContainsCamera();
  ...
  private _fetchMapObjects(clean: MapObject[]): Observable<THREE.Group> {
    const objectsToLoad: Observable<THREE.Group>[] = clean.map(
      (object: MapObject) =>
        this._loadModelFromFileSystem(
          this._getMapObjectNameById(object.model),
        ).pipe(
          map((group: THREE.Group) => {
            ...
            const rotation: number[] = [
              ...
              object.rotZ,
              object.rotY,
            ].map((deg: number) => MathUtils.degToRad(deg)) as number[];
